...
print(a[:-4:-1])  # 9 8 7
print(a[-4:-7:-1])  # 6 5 4				#从倒数第四个->倒数第七个，倒序遍历
print(a[-7::-1])  # 3 2 1
print(a[::])  # 1 2 3 4 5 6 7 8 9
print(a[:])  # 1 2 3 4 5 6 7 8 9
print(a[::3])  # 1 4 7					#正序每隔三个取一个
print(a[1::3])  # 2 5 8
print(a[2::3])  # 3 6 9
```
...
**ndarray数组的掩码操作**(先生成一个数组，然后中括号括起来)
...
多维数组组合与拆分的相关函数：axis(),concatenate(),split()
...
简单的一维数组组合方案row_stack(),column_stack()
...
- nbytes - 总字节数 = size x itemsize	
  - real - 复数数组的实部数组												#只含有数字的部分	
    - imag - 复数数组的虚部数组										#包含在字母的部分数字
- T - 数组对象的转置视图
- flat - 扁平迭代器
...
# 设置坐标轴的颜色
# color: <str> 颜色值字符串
axis.set_color(color)				#（'none'）干掉坐标轴
```
...
#绘制刻度网格线
ax.grid(
    which='',		# 'major'/'minor/both' <-> '主刻度'/'次刻度'/‘两者都’ 
...
mp.pie(
    ...
    '%d%%',			# 标签所占比例格式
	shadow=True, 	# 是否显示阴影
    #startanle=90	# 逆时针绘制饼状图时的起始角度(暂时不可用)
    radius=1		# 半径
)
```
...
# update：		更新函数
# interval：	间隔时间（单位：毫秒）
anim = ma.FuncAnimation(mp.gcf(), update, interval=300)
mp.show()
```
...
(白色为5日均线，黄色为10日均线，紫色是20日均线，绿色是30日均线，青色 是60日均线)

收盘价5日均线：从第五天开始，每天计算最近(五天的收盘价的平均值)所构成的一条线。

移动均线算法：使趋势更加明显（属于降噪手段）
...
使用b作为卷积核，对a数组执行卷积运算的过程

​						44  65  86										#有效卷积

​				23  44  65  86 59									#同维卷积（维度一样）

​			8   23  44  65  86 59  30							#完全卷积

0	0	1	2	3	4	5	0	0  

6	7	8																#平移滑动，对应位置，相乘再相加
...
```python
c = numpy.convolve(a, b, 卷积类型)		#卷及类型
											^
											|
                40  61  82         - 有效卷积(valid)			(卷积核都生效)
...
**使用卷积函数numpy.convolve(a, b, 卷积类型)实现***加权**5日均线**
...
卷积核的选取:从正、负分别取出任意的点，然后进行取值，进行卷积运算，得到一些数据（实现简单的降噪）
...
通过两组统计数据计算而得的协方差可以评估这两组统计数据的**(相似程度)**。
...
cov_ab/(std_a x=bhp_closeing_prices.size
ycov=(dev_bhp*dev_vale).sum()/(n-1)
print('cov样本协方差:',ycov) std_b)
...
$$
\left[ \begin{array}{c}
a与b的相关系数 \frac{var\_a}{std\_a \times std\_a} & a与a的相关系数 \frac{cov\_ab}{std\_a \times std\_b} \\
b与a的相关系数 \frac{cov\_ba}{std\_b \times std\_a} & b与b的相关系数 \frac{var\_b}{std\_b \times std\_b}\\
\end{array} 
\right ]
$$
...
$$
多项式拟合的目的是为了找到(**一组p0-pn**)，使得拟合方程尽可能的与实际样本数据相符合。
...
根据一组样本，并给出最高次幂(次幂越高，数据越复杂)，求出拟合系数
...
多项式函数求导，根据拟合系数求出多项式函数导函数的系数(斜率)
np.polyder(P)->Q 
...
'''
1. 求出多项式的导函数
2. 求出导函数的根，若导函数的根为实数，则该点则为曲线拐点。（驼峰和驼谷）
'''
import numpy as np
import matplotlib.pyplot as mp
...
过拟合:优点:对于预测效果很差														缺点:刚刚好过拟合点拟合度高

欠拟合:优点:对于一些发差很大的数据，能够过度一些小数据	缺点:数据拟合度低，与实际偏差较大
...
#卷积降噪
convolve_core = np.hanning(8)			（中心元素对结果影响最大）
convolve_core /= convolve_core.sum()			#卷积结果等于1（使结果不会收到噪声影响）
...
矩阵是numpy.matrix类类型的对象，该类**(继承)**自numpy.ndarray，任何针对多维数组的操作，对矩阵同样有效，但是作为子类矩阵又结合其自身的特点，做了必要的扩充，比如：乘法计算、求逆等。
...
np.floor_divide(a, b)	# a 地板除 b	（真除的结果向下取整）(负数为向上取整)
np.ceil(a / b) 			# a 天花板除 b	（真除的结果向上取整）矢量化方法
...
print(np.where(c < 0)[0])				#返回符合条件的索引数组
```
...
#已知特征值与特征向量，求方阵
S = np.mat(eigvecs) * np.mat(np.diag(eigvals)) * np.mat(eigvecs.I) #逆
```
...
有一个矩阵M，可以分解为3个矩阵U、S、V，使得U x S x V等于M。U与V都是正交矩阵（自身乘以自身的转置矩阵结果为单位矩阵）。那么S矩阵主对角线上的元素称为矩阵M的奇异值，其它元素均为0。
...
sv[50:] = 0
original2 = np.mat(U) * np.mat(np.diag(sv)) * np.mat(V)			#转成矩阵格式
mp.figure("Lily Features")
mp.subplot(221)
mp.xticks([])
mp.yticks([])
...
通过一个复数数组（复数的模代表的是振幅，复数的辐角代表初相位）经过**（逆向傅里叶变换）**得到**合成的函数值数组**
...
通常在数据分析的数据预处理阶段，会对当前样本数据进行少量的异常值修改与缺失值填充，可以使用random得到符合某种概率分布的一组随机数，完成处理
...
```python
indices = numpy.lexsort((参考序列, 待排序列))/((次次序列,次序列，主序列))
```
...
scipy提供了常见的插值算法可以通过 一组离散点得到符合这组散点分布规律的插值器函数。若我们给插值器函数更多的散点x坐标序列，该函数将会返回相应的y坐标序列。
